...
if (is_not_login()) {
	message_maker('login', 'for logged only');
	...
	<main id="js-page-content" role="main" class="page-content mt-3">
		...
		<div class="row" id="js-contacts">
			<? foreach ($users as $user) : ?>
				...
				<div class="col-xl-4">
					<div id="c_<?= $user['id'] ?>" class="card border shadow-0 mb-g shadow-sm-hover" data-filter-tags="<?= username_to_tag($user['username']) ?>">
						<div class="card-body border-faded border-top-0 border-left-0 border-right-0 rounded-top">
							<div class="d-flex flex-row align-items-center">
								<span class="status status-<?= show_status($user['status']) ?> mr-3">
									<span class="rounded-circle profile-image d-block " style="background-image:url('<?= AVATARS_HTML . $user['avatar'] ?>'); background-size: cover;"></span>
								</span>
								<div class="info-card-text flex-1">
									<? if (is_admin() or user_identification($user['id'])) : ?>
										...
										<div class="dropdown-menu">
											...
											<a href="/delete_handler.php?id=<?= $user['id'] ?>" class="dropdown-item" onclick="return confirm('are you sure?');">
												<i class="fa fa-window-close"></i>
												Удалить
											</a>
										</div>
									<? else : ?>
										<span class="fs-xl text-truncate text-truncate-lg text-info"><b><?= $user['username'] ?></b></span>
									<? endif ?>
									<span class="text-truncate text-truncate-xl"><?= $user['work_place'] ?></span>
									<a href="/page_profile.php?id=<?= $user['id'] ?>" >Профиль</a>
								</div>
								<!--  с этим блоком так и не разобрался, должна быть какая-то кнопка? -->
						...
						<div class="card-body p-0 collapse show">
							<div class="p-3">
								<a href="tel:+<?= telephone($user['phone']) ?>" class="mt-1 d-block fs-sm fw-400 text-dark">
