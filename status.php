...
if (!is_admin() or !user_identification($_GET['id'])) {
	message_maker('security', 'not access');
									...
									<div class="col-md-4">
										...
										<div class="form-group">
											...
											<select name="status" class="form-control" id="example-select">
												<option <? echo ($user['status'] == 'online') ? 'selected' : '' ?> value="online">Онлайн</option>
												<option <? echo ($user['status'] == 'away') ? 'selected' : '' ?>value="away">Отошел</option>
												<option <? echo ($user['status'] == 'not worry') ? 'selected' : '' ?> value="not worry">Не беспокоить</option>
											</select>
										</div>
									</div>
